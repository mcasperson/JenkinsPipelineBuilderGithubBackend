...
public class JavaMavenBuilder implements PipelineBuilder {
    ...
    private Element createBuildStep() {
        return Function1ArgTrailingLambda.builder()
                ...
                .children(GIT_BUILDER.createStepsElement(new ImmutableList.Builder<Element>()
                        .add(Comment.builder()
                                .content("Bump the minor version for the artifact.")
                                .build())
                        .add(Comment.builder()
                                .content("An alternative here is to set the build number.")
                                .build())
                        .add(Comment.builder()
                                .content("sh (script: '" + mavenExecutable() + " --batch-mode versions:set build-helper:parse-version versions:set -DnewVersion=\\\\${parsedVersion.majorVersion}.\\\\${parsedVersion.minorVersion}.\\\\${parsedVersion.incrementalVersion}.${BUILD_NUMBER}')")
                                .build())
                        .add(FunctionManyArgs.builder()
                                .name("sh")
                                .args(new ImmutableList.Builder<Argument>()
                                        .add(new Argument("script", mavenExecutable() + " --batch-mode build-helper:parse-version versions:set -DnewVersion=\\\\${parsedVersion.majorVersion}.\\\\${parsedVersion.minorVersion}.\\\\${parsedVersion.nextIncrementalVersion}", ArgType.STRING))
                        ...
                        .add(FunctionManyArgs.builder()
                                .name("sh")
                                .args(new ImmutableList.Builder<Argument>()
                                        .add(new Argument("script", mavenExecutable() + " --batch-mode clean compile", ArgType.STRING))
                                        .add(new Argument("returnStdout", "true", ArgType.BOOLEAN))
                                        .build())
                                .build())
                        .build()))
